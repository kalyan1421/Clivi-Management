...
class VendorDetailTotalsScreen extends ConsumerWidget {
  ...
  const VendorDetailTotalsScreen({super.key, required this.vendorId, required this.vendorName});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final totalsAsync = ref.watch(vendorMaterialTotalsProvider(VendorTotalsRequest(vendorId)));
    ...
    return Scaffold(
      ...
      body: totalsAsync.when(
        data: (rows) {
          if (rows.isEmpty) {
            return const Center(child: Text('No material data for this vendor yet'));
          }
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final totalIn = entries.fold<double>(0, (s, e) => s + e.totalInward);
              final totalOut = entries.fold<double>(0, (s, e) => s + e.totalOutward);
              final net = entries.fold<double>(0, (s, e) => s + e.net);

              return ExpansionTile(
                title: Text(material, style: const TextStyle(fontWeight: FontWeight.bold)),
                subtitle: Text('In: ${totalIn.toStringAsFixed(2)} • Out: ${totalOut.toStringAsFixed(2)} • Net: ${net.toStringAsFixed(2)}'),
                children: entries
                    .map(
                      (e) => ListTile(
                        title: Text(e.projectName),
                        subtitle: Text('In: ${e.totalInward.toStringAsFixed(2)}  Out: ${e.totalOutward.toStringAsFixed(2)}'),
